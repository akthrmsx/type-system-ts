...
pub fn run(tm: &Tm, env: &HashMap<String, Ty>) -> Result<Ty, Error> {
    match tm {
        ...
        Tm::RecursiveFunction {
            ...
            let function = Ty::function(parameters.clone(), return_ty.clone());
            let mut env = env.clone();
            env.insert(name.clone(), function);
            let mut body_env = env.clone();
            for parameter in parameters.iter() {
                body_env.insert(parameter.name.clone(), parameter.ty.clone());
            }
            if !return_ty.equals(&run(body, &body_env)?) {
                return Err("戻り値の型が異なります。".into());
            }
            run(next, &env)
        }
    }
